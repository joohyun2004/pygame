...
#화면 사이즈 설정
screen_width = 640 #가로사이즈
screen_height = 480 # 세로 사이즈
screen = pygame.display.set_mode((screen_width, screen_height))

#화면 타이틀 설정
pygame. display.set_caption("pang GAME") #게임타이틀
...
#이벤트 루프
running = True # 게임지 진행중인지
while running:
    dt = clock.tick(30)  # 게임화면의 초당 프레임수 설정

    #2. 이벤트 처리 (키보드, 마우스 등)
    for event in pygame.event.get():
        if event.type == pygame.QUIT: # 창닫기 이벤트
            running = False
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로 이동
                ...
                if weapon_count < weapon_max_count:
                    weapons.append([weapon_x_pos, weapon_y_pos])
                    weapon_count +=1 

        if event.type == pygame.KEYUP: #키보드에서 손을 땔때
    ...
    for w in weapons : 
        if w[1] == 10 : # 천장에 닿았을 경우 무기 수를 다시 설정
            weapon_count -=1
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            continue #안쪽 for 조건이 맞지 않으면  continue.
        break #안쪽 for 문에서 break 를 만나면 2중 for 탈출
